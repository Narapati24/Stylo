...
>
    ...
    <div>
        ...
        @if($category->image)
            <div class="mb-2">
                <img src="{{ $category->image_url }}" alt="Current Image" class="h-20 w-auto border border-secondary rounded-lg">
            </div>
        @endif
    ...
    <div>
        ...
        <textarea name="description" id="description" rows="4" 
            class="w-full border-secondary focus:border-primary focus:ring-0 rounded-lg bg-bone px-4 py-2">{{ old('description', $category->description) }}</textarea>
    ...
    <div class="flex items-center gap-2">
        ...
        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }}
            class="rounded border-secondary text-primary focus:ring-0 w-5 h-5">
